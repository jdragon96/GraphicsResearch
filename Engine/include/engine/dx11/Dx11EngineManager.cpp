...
Dx11EngineManager::Dx11EngineManager() : whichButtonTypePressed(MouseButtonType::NONE), isMousePressedFlag(false)
{
	{
	...
	}
	{
		...
		dsOption.Width = m_option.width;
		dsOption.Height = m_option.height;
...
void Dx11EngineManager::SetScreenOption(RendererOption option)
{
	m_option = option;
}
...
void Dx11EngineManager::InitDeviceAndSwapChain(DXGI_SWAP_CHAIN_DESC desc)
{
	...
	scOption.BufferDesc.Width = m_option.width;  //> ����� ũ�� ����
	scOption.BufferDesc.Height = m_option.height;
	desc.SampleDesc.Count = 1;
	desc.SampleDesc.Quality = 0;
	...
	Microsoft::WRL::ComPtr<ID3D11DepthStencilView>& depthView, D3D11_TEXTURE2D_DESC ds)
{
	...
	ds.Width = m_option.width;  //> 백버퍼 크기 설정
	ds.Height = m_option.height;
...
void Dx11EngineManager::InitImGui()
{
	...
	(void)io;
	io.DisplaySize = ImVec2(float(m_option.width), float(m_option.height));
