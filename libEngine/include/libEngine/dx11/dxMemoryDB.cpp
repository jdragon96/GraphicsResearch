...
void dxTexture::AddImage(std::string path)
{
  ...
  //     devPtr.Get(), LPath.c_str(), 0, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0,
  //     D3D11_RESOURCE_MISC_TEXTURECUBE,  // 큐브맵용 텍스춰
  //     DirectX::DDS_LOADER_FLAGS(false), (ID3D11Resource**)texturePtr.GetAddressOf(), viewPtr.GetAddressOf(), nullptr);
  // if (FAILED(hr))
  //{
  //   std::string exceptionString = "DSS 파일 로드 실패 => " + path;
